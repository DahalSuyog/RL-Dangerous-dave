"""Jev-backed controller for DangerousDaveEnv.

Jev (TypeSafe AI) is a text/JSON-in, typed-judgment-out model - not a vision model and not
built for arithmetic or multi-hop spatial reasoning. So this module does the geometry itself
(reading the level's tile grid and labelling what's near Dave in relative, human-readable
terms) and only asks Jev to make the actual judgment call: which of the 7 discrete actions
to take next.
"""
...
# Jev only knows what a tile *is* from these engine-internal id strings (e.g. "tunnel" reads
# as an inviting passage, but here it's a wall) - so every nearby tile is labelled with what it
# actually does to Dave, not left for Jev to guess from the name.
TILE_CATEGORY = {
...
# hurts these models, per TypeSafe's own docs.
WINDOW_BEHIND = 1
WINDOW_AHEAD = 6
WINDOW_UP = 2
WINDOW_DOWN = 2
...
class JevAgent:
    ...
    def __init__(self, confidence_threshold=DEFAULT_CONFIDENCE_THRESHOLD):
        ...
        self._previous_action = 6  # noop until Jev makes its first call
        # Latest decision, kept for the on-screen HUD (see play_jev.py):
    ...
    def build_state(self, env):
        ...
        for dx in x_range:
            for dy in range(-WINDOW_UP, WINDOW_DOWN + 1):
                ...
                if tile_id in (None, "scenery", "undefined"):
                    continue  # out of bounds or empty background - not worth Jev's attention
                nearby_tiles.append(
                    {"offset": _offset_words(dx, dy), "tile": tile_id, "what_it_does": _classify_tile(tile_id)}
                )

        # Coarse heuristics computed in code (not left for Jev to infer from raw geometry):
        ...
        # ground (open air/scenery). Jev still sees the full nearby_tiles list to cross-check.
    ...
    def act(self, env):
        ...
        try:
            ...
            logger.warning("Jev call failed (%s); repeating previous action", error)
            self.last_decision = {
        ...
        # With 7 roughly-plausible options, Jev's confidence (a concentration measure, not the
        # raw top probability) commonly sits well below what a binary/few-option question would
        # produce - even for a perfectly reasonable pick. Since a wrong move here just costs a
        # life and respawns Dave (cheap to recover from), we still act on the top choice and
        # only use confidence to flag it as uncertain for the HUD, rather than freezing gameplay
        # by falling back to the previous action - that caused Dave to get stuck standing still
        # once the surroundings stopped changing.
        low_confidence = answer.confidence < self.confidence_threshold
        self.last_decision = {

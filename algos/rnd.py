...
import torch.nn.functional as F   #for forward pass and softmaxing
import torch.optim as optim         #optimization for loss function
...
class Agent(nn.Module):
    ...
    def get_value(self, x):
        #only provides extrinsic and intrinsic value
        hidden = self.network(x / 255.0)
        features = self.extra_layer(hidden)
...
class RNDModel(nn.Module):
    #implemnenting rnd for intrinsic value
